...
merge:
    ...
    mul $t3,$t0,4	        # $t3 = i*4
    add $t3,$t3,$s0         # adds a shift to the base address of N and stores in t3

	addi $t1,$a3,1  	    # initial element of the right sub-array --j = mid + 1
    mul $t5,$t1,4	        # $t5 = j*4
    add $t5,$t5,$s0         # adds a shift to the base address of N and stores in t5

    li $s6,0                # index in the temporary array where we insert -- from the start
    mul $t7,$s6,4           # store 4xs6 into t7
    add $t7,$t7,$s4         # find the required location in the temporary array -- store it in t7

    mul $s7,$t0,4           # stores 4xt0 into $s7
    add $s7,$s7,$s0         # find the required location in the original array N -- this we use while copying
    
    move $t2,$t0            # a counter for the total number of elements while merging. starts at left index a1, max is right index a2
    
    sub $s5,$a2,$a1         # create the iterator limit for the copy block which starts from s6 = 0 and goes to s6 = right-left
...
mainloop:
    ...
    addi $t5,$t5,4  # increment address of j 
    addi $t1,$t1,1  # increment the value of index j
    addi $t7,$t7,4  # increment t7 -- pointer to address of temporary array
    addi $t2,$t2,1  # increment the counter index
...
leftsmaller: 
    ...
    addi $t3,$t3,4  # increment address of i 
    addi $t0,$t0,1  # increment the value of index i
    addi $t7,$t7,4  # increment t7 -- pointer to address of temporary array
    addi $t2,$t2,1  # increment the counter index
...
rightinsert:
    ...
    lw  $t4,($t5)   # loads the value into temporary register from right subarray
    sw $t4,($t7)    # stores the value from t4 into the location in the temporary array

    addi $t5,$t5,4  # increment address of j
    addi $t1,$t1,1  # increment the index j
    addi $t7,$t7,4  # increment t7 -- pointer to location in temporary array
...
leftinsert:
    ...
    lw  $t4,($t3)   # loads the value into temporary register 
    sw  $t4,($t7)   # loads the value into the address in the temporary array
...
copy:
    bgt $s6,$s5,exit     # if iterator is bigger than the rightmost value, we are fully copied so we exit

    #load the value stored at s6 index in temporary array -- we don't need the counter anymore we can freely overwrite it 
    mul $t2,$s6,4	# $t2 = s6*4
    add $t2,$t2,$s4 # adds a shift to the base address of temporary array
    lw  $t4,($t2)   # loads the value into temporary register t4

    #loads the value into the corresponding index in the original array
    sw $t4,($s7)    # stores the value from t4 into the address in the original array

    #increment the iterator for the temporary array and the original array address pointer
    addi $s6,$s6,1
    addi $s7,$s7,4
